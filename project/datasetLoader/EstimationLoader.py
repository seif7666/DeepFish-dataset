...
class EstimationLoader:
    ...
    def __getitem__(self, idx):
        ...
        second_time = time()
        print(f"Loading Index: f{second_time-first_time:0.2f}")
        image = self.__loadImage(filename + ".jpg")
        annotations = self.__dataset.loc[self.__dataset["file"] == filename]
        third_time = time()
        print(f"Loading Annotations: f{third_time-second_time:0.2f}")
        ...
        fourth_time = time()
        print(f"Loading Boxes: {fourth_time-third_time:0.2f}")
