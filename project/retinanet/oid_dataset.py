...
def get_labels(metadata_dir, version="v4"):
    if version == "v4" or version == "challenge2018":
        ...
        id_to_labels = dict(
            [(i, description_table[c]) for i, c in enumerate(trainable_classes)]
        )
...
def generate_images_annotations_json(
    main_dir, metadata_dir, subset, cls_index, version="v4"
):
    ...
    with open(annotations_path, "r") as csv_file:
        ...
        for line, row in enumerate(reader):
            ...
            if y2_int == y1_int:
                warnings.warn(
                    "filtering line {}: rounding y2 ({}) and y1 ({}) makes them equal".format(
                        line, y2, y1
                    )
                )
                continue
            ...
            if x2_int == x1_int:
                warnings.warn(
                    "filtering line {}: rounding x2 ({}) and x1 ({}) makes them equal".format(
                        line, x2, x1
                    )
                )
                continue
...
class OidDataset(Dataset):
    ...
    def __init__(
        self, main_dir, subset, version="v4", annotation_cache_dir=".", transform=None
    ):
        if version == "v4":
            metadata = "2018_04"
        elif version == "challenge2018":
