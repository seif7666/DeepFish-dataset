...
from datasetLoader.dataloader import (
...
def evaluate(
    model: torch.nn.Module, data: dict, unnormalize: UnNormalizer, classes: np.ndarray
):
    # model.cuda()
    scores, classification, transformed_anchors = model(
        torch.unsqueeze(data["img"], dim=0).float()
    )
...
def main():
    dataset = EstimatedDeepFish(
        ".\size_estimation_homography_DeepFish.csv",
        ".\DATASET\\",
        transforms.Compose([Normalizer(), Resizer(480, 480), Permuter()]),
        False,
    )
    model = PipelineModel(13)
    model_params = torch.load("..\\bestModel.pt", torch.device("cpu"))
...
def test():
    dataset = EstimatedDeepFish(
        "../Project/size_estimation_homography_DeepFish.csv",
        "../Project/DATASET/",
        transforms.Compose([Normalizer(), Resizer(480, 480), Permuter()]),
        False,
    )
