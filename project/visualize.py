...
def evaluate(model:torch.nn.Module,data:dict,unnormalize:UnNormalizer,classes:np.ndarray):
    # model.cuda()
    scores, classification, transformed_anchors = model(torch.unsqueeze(data['img'],dim=0).float())
...
def main():
    dataset=EstimatedDeepFish('.\size_estimation_homography_DeepFish.csv','.\DATASET\\',transforms.Compose([Normalizer(), Resizer(480,480),Permuter()]),False)
